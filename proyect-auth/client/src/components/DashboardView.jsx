...
const DashboardView = ({ schedule = [], onNavigate, user, showAcademic = true }) => (
      ...
      <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-xl">
        {showAcademic ? (
          ...
          <div className="space-y-2">
            <p className="text-xs uppercase tracking-[0.3em] text-emerald-200">Visita Externa</p>
            <h3 className="text-lg font-bold text-slate-900 text-white">Actividad en curso</h3>
            <p className="text-sm text-slate-200">Disfruta de la experiencia guiada. Puedes explorar hotspots del campus y revisar servicios generales.</p>
          </div>
        )}
      </div>
    </div>
    {showAcademic ? (
      <div className="px-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
            <Card
              ...
              className="flex flex-col items-center justify-center gap-2 hover:bg-slate-50 cursor-pointer border border-transparent hover:border-slate-200 transition-all"
            >
      ...
      <div className="px-4 space-y-4">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 space-y-3">
          <h3 className="font-bold text-slate-800 text-lg">Recorrido del Visitante</h3>
          <p className="text-sm text-slate-500">Descubre los puntos emblemáticos del campus y accede a servicios como cafeterías, baños y puntos de información.</p>
          <button
            onClick={() => onNavigate('map')}
            className="text-xs uppercase tracking-[0.4em] text-slate-900 font-bold border border-slate-900 rounded-full px-4 py-2"
          >
            Ver mapa de la visita
          </button>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Card
            onClick={() => onNavigate('map')}
            className="flex flex-col items-center justify-center gap-2 border border-slate-200 cursor-pointer"
          >
            <div className="bg-emerald-100 p-2 rounded-full text-emerald-600 text-lg">☕</div>
            <span className="text-sm font-bold text-slate-600">Cafeterías</span>
          </Card>
          <Card
            onClick={() => onNavigate('map')}
            className="flex flex-col items-center justify-center gap-2 border border-slate-200 cursor-pointer"
          >
            <div className="bg-blue-100 p-2 rounded-full text-blue-600 text-lg">🛎️</div>
            <span className="text-sm font-bold text-slate-600">Atención Visitantes</span>
          </Card>
        </div>
      </div>
    )}
